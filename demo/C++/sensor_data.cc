...
int main(int argc, const char **argv) {
  ...
  m = mj_loadXML("../../../ray_caster.xml", 0, error, 1000);
  ...
  auto [h_ray_num, v_ray_num, data_pairs] =
      get_ray_caster_info(m, d, "raycastercamera");
  ...
  /* sensor id */
  int sensor_id = mj_name2id(m, mjOBJ_SENSOR, "raycastercamera");
  if (sensor_id == -1) {
    std::cout << "no found sensor" << std::endl;
    return 0;
  }
  int data_pos = m->sensor_adr[sensor_id];
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    cv::Mat image(v_ray_num, h_ray_num, CV_64F,
                  d->sensordata + data_pos + data_pairs[0].first);
    cv::Mat inv_image(v_ray_num, h_ray_num, CV_64F,
                      d->sensordata + data_pos + data_pairs[1].first);
    image.convertTo(image, CV_8U);
    inv_image.convertTo(inv_image, CV_8U);
    cv::resize(image, image, cv::Size(h_ray_num * 10, v_ray_num * 10));
    cv::resize(inv_image, inv_image, cv::Size(h_ray_num * 10, v_ray_num * 10));
    cv::imshow("image", image);
    cv::imshow("inv_image", inv_image);
    cv::waitKey(1);
